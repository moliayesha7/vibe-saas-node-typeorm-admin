...
const sendEmail = async ({ to, subject, html, text }) => {
  if (process.env.NODE_ENV === 'test') return { messageId: 'test-id' };
  try {
...
const sendPasswordResetEmail = (user, resetToken) => sendEmail({
  to: user.email,
...
const sendPaymentConfirmEmail = (user, payment) => sendEmail({
  to: user.email,
  ...
  html: `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <table style="width:100%; border-collapse: collapse; margin-top: 16px;">
        <tr><td style="padding:8px; border:1px solid #e5e7eb;">Transaction ID</td><td style="padding:8px; border:1px solid #e5e7eb;">${payment.transaction_id}</td></tr>
        <tr><td style="padding:8px; border:1px solid #e5e7eb;">Amount</td><td style="padding:8px; border:1px solid #e5e7eb;">${payment.amount} BDT</td></tr>
